...
export default function ConsultationModal({ isOpen, onClose }: ConsultationModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTimeout(() => {
        setIsSuccess(false);
        setFormData({ name: '', phone: '', service: 'PVC Panels & UV Sheets' });
        onClose();
      }, 3000);
    } catch (err) {
                  ...
                  <form className="space-y-4" onSubmit={handleSubmit}>
                    ...
                    <div>
                      ...
                      <select 
                        ...
                        className="w-full border-b border-luxury-ink/20 bg-transparent py-2 focus:border-luxury-gold focus:outline-none transition-colors"
                      >
                        <option>PVC Panels & UV Sheets</option>
                        <option>Designer Wallpapers</option>
                        <option>Interior Decoration</option>
                        <option>Flooring & Blinds</option>
                        <option>Custom Art & Paint</option>
                      </select>
                    </div>
